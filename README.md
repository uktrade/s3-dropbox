...
Clients must authenticate requests by passing an `authorisation` header in the format `Bearer <token>`, where `<token>` is the plain text token generated by the [create_token.py](./create_token.py) script.
